...
-- Types, procedures and functions
  ...
  isChoosing: array (0 .. Nr_Of_Processes-1) of Integer range 0 .. 1 := (others => 0);
  Tickets: array (0 .. Nr_Of_Processes-1) of Integer := (others => 0);

  pragma Volatile(isChoosing);
  pragma Volatile(Tickets);
  ...
  task body Process_Task_Type is
    ...
    function MaxCurrentTicket return Integer is
      Current : Integer := 0;
    begin
      for N in Tickets'Range loop
        if Tickets(N) > Current then
          Current := Tickets(N);
        end if;
      end loop;
      return Current;
    end MaxCurrentTicket;
  ...
  begin
    ...
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      isChoosing(Process.Id) := 1; 
      Tickets(Process.Id) := 1 + MaxCurrentTicket;
      isChoosing(Process.Id) := 0;
      if Tickets(Process.Id) > Local_Max_Ticket then
        Local_Max_Ticket := Tickets(Process.Id);
      end if;
      for J in 0 .. Nr_Of_Processes-1 loop
        if J /= Process.Id then
          loop exit when isChoosing(J) = 0; end loop;
          loop exit when
            (Tickets(J) = 0) or
            (Tickets(Process.Id) < Tickets(J)) or
            (Tickets(Process.Id) = Tickets(J) and Process.Id < J);
          end loop;
        end if;
      end loop;
